...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        for DiscreteDistribution in livingGhostPositionDistributions:
            temporaryPos = DiscreteDistribution.argMax()
            tempDistribution = self.distancer.getDistance(pacmanPosition, temporaryPos)
            if tempDistribution < distance:
                distance = tempDistribution
                ghost_position = temporaryPos

        distance = float('inf')
        action = None
        for a in legal:
            successorPos = Actions.getSuccessor(pacmanPosition, a)
            temp = self.distancer.getDistance(successorPos, ghost_position)
            if temp < distance:
                distance = temp
                action = a
        return action
